//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace StajApp.template (1).MaterialSkin-master.MaterialSkinExample.RDSPORTAL_Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class VE_PPROCONFOPSPROST
    {
        public int COMPANYID { get; set; }
        public string PPROCONFOPSPRONO { get; set; }
        public string PPROCONFOPSPRONAME { get; set; }
        public int PPROCONFOPSPROID { get; set; }
        public int ITEMNO { get; set; }
        public string STOCKNO { get; set; }
        public string STOCKNAME { get; set; }
        public string STOCKTYPE { get; set; }
        public int STOCKID { get; set; }
        public string GRADENAME { get; set; }
        public int GRADEID { get; set; }
        public Nullable<double> QUANTITY { get; set; }
        public string UNITNAME { get; set; }
        public string UNITIDNAME { get; set; }
        public string INTUNITCODE { get; set; }
        public Nullable<int> UNITID { get; set; }
        public string GROUPCODE { get; set; }
        public string SPECCODE1 { get; set; }
        public string SPECCODE2 { get; set; }
        public string SPECCODE3 { get; set; }
        public string DESCRIPTION { get; set; }
        public string DEPOTNO { get; set; }
        public string DEPOTNAME { get; set; }
        public string DEPOTTYPE { get; set; }
        public Nullable<int> DEPOTID { get; set; }
        public Nullable<double> TOLUP { get; set; }
        public Nullable<double> TOLDOWN { get; set; }
        public Nullable<double> PERCENTCOST { get; set; }
        public string PDESIGNNAME { get; set; }
        public Nullable<int> PDESIGNID { get; set; }
        public Nullable<int> INSERTUSERID { get; set; }
        public Nullable<System.DateTime> INSERTDATE { get; set; }
        public Nullable<int> EDITUSERID { get; set; }
        public Nullable<System.DateTime> EDITDATE { get; set; }
        public Nullable<int> GLBALERTID { get; set; }
    }
}
