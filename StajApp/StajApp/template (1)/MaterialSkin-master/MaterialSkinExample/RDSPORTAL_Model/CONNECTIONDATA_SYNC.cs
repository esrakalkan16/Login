//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace StajApp.template (1).MaterialSkin-master.MaterialSkinExample.RDSPORTAL_Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class CONNECTIONDATA_SYNC
    {
        public int COMPANYID { get; set; }
        public int CPERIODID { get; set; }
        public int CONNECTIONID { get; set; }
        public int ROOT { get; set; }
        public int ROOTID { get; set; }
        public Nullable<int> ROOTITEMNO { get; set; }
        public Nullable<int> HREFROOT { get; set; }
        public Nullable<int> HREFROOTID { get; set; }
        public Nullable<int> HREFROOTITEMNO { get; set; }
        public string SPECCODE1 { get; set; }
        public string SPECCODE2 { get; set; }
        public string SPECCODE3 { get; set; }
        public string GROUPCODE1 { get; set; }
        public string GROUPCODE2 { get; set; }
        public string GROUPCODE3 { get; set; }
        public Nullable<System.DateTime> INSERTDATE { get; set; }
        public Nullable<int> INSERTUSERID { get; set; }
        public Nullable<System.DateTime> EDITDATE { get; set; }
        public Nullable<int> EDITUSERID { get; set; }
        public string HREFROOTRECEIPTNO { get; set; }
    }
}
